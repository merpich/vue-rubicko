...
<template>
	<RouterLink class="flex gap-4 items-center text-slate-900 hover:text-blue-600 transition-colors" :to="`/${userData.userName}`">
		...
		<div>
			...
			<p class="text-sm" v-if="userData.fullName">{{ userData.fullName }}</p>
		</div>
	</RouterLink>
</template>
